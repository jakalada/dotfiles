...
if s:iswin
  ...
  let $VIMCONFIGDIR = expand('~/ghq/github.com/jakalada/dotfiles')
else
...
" --------------------------------------------------
" フォーマッター {{{2
Plug 'prettier/vim-prettier', { 'do': 'yarn install' } " pretteirがサポートしているファイル形式のプラグインを合わせてインストール
...
Plug 'kristijanhusak/vim-dirvish-git'  " dirvishでファイルごとにGitの状態を表示
...
" --------------------------------------------------
" 依存関係の解決のために最後に記述するプラグイン {{{2
Plug 'ryanoasis/vim-devicons'          " 各種プラグインにNerd Fontを設定してくれる
...
MyAutocmd BufWritePre *.js,*.jsx,*.mjs,*.ts,*.tsx,*.css,*.less,*.scss,*.json,*.graphql,*.vue,*.yaml,*.html PrettierAsync
